...
>
	...
	>
		<img src={image} alt="project" class="absolute h-full w-full object-contain" />
		<div class="absolute top-0 right-0 h-full w-full" />
	</div>

	<div class="space-y-2 sm:space-y-3 md:space-y-3 lg:space-y-4">
		<h1 class="mt-5 cursor-default text-2xl font-bold sm:text-3xl lg:mt-0 lg:text-4xl 2xl:text-5xl">
			<div class="m-0 flex items-center overflow-hidden lg:h-[52px] lg:justify-end 2xl:h-[60px]">
				<div>{name}</div>
			</div>
		</h1>
		<p class="cursor-default text-sm font-normal sm:text-xl lg:text-right xl:text-2xl 2xl:text-3xl">
			{description}
		</p>
